...
class Speed_Contact_Bar {
	...
	public function include_contact_bar() {
		if ( ! ( $this->is_login_page() or is_admin() ) ) {
			...
			if ( isset( $this->stored_settings[ 'address' ] ) and '' != $this->stored_settings[ 'address' ] ) {
				if ( isset( $this->stored_settings[ 'address_url' ] ) and '' != $this->stored_settings[ 'address_url' ] ) {
					$contact_list[] = sprintf( 
						...
						$this->current_icon_size,
						__( 'Postal address', $this->plugin_slug ),
			...
			if ( isset( $this->stored_settings[ 'phone' ] ) and '' != $this->stored_settings[ 'phone' ] ) {
				$link_text = esc_html( $this->stored_settings[ 'phone' ] );
				$link_href = $this->esc_phonenumber( $this->stored_settings[ 'phone' ] );
				$contact_list[] = sprintf( 
	...
	public function display_activation_message () {
		...
		if ( is_rtl() ) {
			...
			$link = sprintf(
				'<a href="%s">%s %s %s</a>',
				esc_url( admin_url( sprintf( 'options-general.php?page=%s', 'speed-contact-bar' ) ) ),
				esc_html__( $text ),
				$sep,
				$this->plugin_name
			);
